...
class AIAgentForm extends Component {
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (data.answer && data.answer.includes("DATABASE_OPERATION:")) {
                try {
                    const operationStr = data.answer.split("DATABASE_OPERATION:")[1].trim().split('\n')[0];
            ...
            this.state.messages.push({ content: data.answer, isUser: false });
